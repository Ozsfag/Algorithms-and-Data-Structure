...
public class TextEditor {
  ...
  public void insertCharacterAt(int position, char c) {
    ...
    for (int k = text.length - 1; k > position; k--) {
      text[k] = text[k - 1];
    }

    if (textLength > position) text[position] = c;
    else if (textLength == position) {
      var newLength = 0;
      for (char value : text) {
        if (!Character.isSpaceChar(value)) {
          newLength++;
      ...
      if (newLength >= position) text[newLength] = c;
    }
  ...
  public void backspace(int position) {
    ...
    if (position >= 0) {
      for (int k = position; k < text.length - 1; k++) {
        text[k] = text[k + 1];
      }
    }
  }
