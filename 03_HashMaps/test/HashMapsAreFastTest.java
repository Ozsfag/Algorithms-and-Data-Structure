...
import homework.HashMapsAreFast.HashMapsAreFast;
...
public class HashMapsAreFastTest {
  ...
  static Stream<Arguments> phonesNumbers() {
    return Stream.of(
        ...
        Arguments.of(
            new long[] {2223L, 102L, 2223L, 102L, 4589L, 6803L, 102L},
  ...
  void hashString(long[] phones, long[] uniquePhones) {
    ...
    assertArrayEquals(
        ...
        actual,
        String.format(
            "%nexpected:%s%nactual:  %s%n%n",
            Arrays.toString(uniquePhones), Arrays.toString(actual)));
  }
  ...
  private static final int[] bigArray = createBigArray();
  ...
  private static int[] createBigArray() {
    ...
    return IntStream.range(0, amount).map((i) -> numbers[random.nextInt(numbers.length)]).toArray();
  }
